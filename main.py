...
async def process_file(file: UploadFile):
	...
	p, button {
        ...
        <div><img src="/static/dhr-logo.png" alt = "DHR Logo" width = "320px" height = "87.5px"></div>
            <h2>File  uploaded!</h2>
            <p>You uploaded: {filename}<br></p>""".format(filename = _filename)
...
async def read_root():
    names = ["Monica Estrada", "Daisy Colin", "Eric Gaona", "Juan I. Recio"]  # Fetch unique names with: SELECT DISTINCT name FROM qa
	...
	p, button {
        ...
        <body>
        <div><img src="/static/dhr-logo.png" alt = "DHR Logo" width = "320px" height = "87.5px"></div>
        <div><h2>Daily QA Counts (as of %s)</h2></div>
<table>
                <tr>
                    <th>Name</th>
                    <th>QA Count Today</th>
...
def get_trainer_files(trainer) -> list[tuple]:
    con = psycopg2.connect(f'dbname = {CONFIG['credentials']['dbname']} user = {CONFIG['credentials']['username']} password = {CONFIG['credentials']['password']} host = {CONFIG['credentials']['host']}')
    cur = con.cursor()
    SQL = "SELECT filename FROM qa WHERE trainer = %s;"
